...
Using the probabilities associated with this Markov chain model, the program will model the estimated growth of the
security price through a random walk. This program can be used as a tool to analyze securities, like stocks and
...
def getLastClosingDate(adjusted_df):
    return adjusted_df['Adjusted Close'].iloc[-1]
...
def random_walk_norm_pdf(adjusted_df, num_periods=12):
    ...
    for i in range(100):
        random_walk = [getLastClosingDate(adjusted_df)]  # walk starts with the last closing date
...
def rand_walk_3x3_markov(adjusted_df, prob_list, security, num_bins=10, lower_thresh=-1.0, upper_thresh=1.0,
    ...
    pct_change_list = []
    all_walks = []  # will hold all the random walk data
    start = getLastClosingDate(adjusted_df)# walk starts with the last closing date
    for i in range(1000):
        mean, std = get_params_for_norm_dist(adjusted_df)
        first_pct_change = np.random.normal(mean, std)
        pct_change_list.append(first_pct_change)
