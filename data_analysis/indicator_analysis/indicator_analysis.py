...
from data_analysis.indicator_analysis.Indicator import Indicator
...
class Symbol:
    def __init__(self,open_cost_rate,close_cost_rate,slippage,size_value):
        self.open_cost_rate = open_cost_rate
        self.close_cost_rate = close_cost_rate
...
class IndicatorAnalyst(object):
    ...
    def _group_apply_func(self, x, _direction=1, my_func=None, arg_func=None, _profit_mode=True, apply_price="open",
        ...
        # 分析的数据的选择
        if _profit_mode:
            group_data = self._future_profit(x, symbol, _direction, apply_price, in_position)
        else:
    ...
    def _future_profit(x, symbol, _direction, apply_price, in_position):
        ...
        if symbol.open_cost_rate != 0.0:
            ...
            open_cost = symbol.open_cost_rate * x[apply_price].iat[in_position] * symbol.size_value
        if symbol.close_cost_rate != 0.0:
            close_cost = [symbol.close_cost_rate * price * symbol.size_value for price in x[apply_price]]
        return symbol.size_value * (_direction * (x[apply_price] - x[apply_price].iat[in_position]) - symbol.slippage
    ...
    def _dict_group_analyst(self, _profit_mode, _direction, _group_plot, _applied_price, _axe):
        ...
        for key in self.__group:
            ...
            if _profit_mode:
                profit = self._future_profit(data_, self.__symbol, _direction, _applied_price,
                                                                      in_position)
...
if __name__ == '__main__':
    ...
    symbol = 'RB1801'
    # 计算指标
    data = data_load(db_name, symbol, start_date, end_date)
    data.index = data.datetime
    # print('data: {}'.format(data))
    print('data_length: {}'.format(len(data)))
    analyst = IndicatorAnalyst(data)
    # analyst.indicator = Indicator().mt4_rsi(data, 14, 'close')
    analyst.indicator = Indicator(data_set=data, applied_price='close').i_bias(24)
    # 分组统计分析
    analyst.interval_analyst(lambda x: True if x < 0.35 else False,
                             symbol=Symbol(slippage=1, size_value=10, open_cost_rate=0.0001, close_cost_rate=0.0001),
                             profit_mode=True,
                             group_plot=True)
