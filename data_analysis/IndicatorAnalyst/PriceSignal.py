...
def criterion_method(t_series, interval, vib_percentage, time_unit='M', convert_trading_signals=True):
    """
    reference:
    ...
    while i < len(landmark):
        ...
        # 满足条件的landmark点
        if (time_list[pos] - time_list[last_pos]).total_seconds() / time_trans_unit[time_unit] < interval and\
...
if __name__ == '__main__':
    ...
    reader.read_csv(['../Data/TestData/XAUUSD1.csv'], symbol_name=['XAUUSD'], start='2014.01.01',
                    end='2015.05.15', skiprows=2000000)
    fig, ax = plt.subplots()
    reader.data['XAUUSD']['open'].plot(ax=ax, legend=False)
    # new_series, _ = criterion_method(reader.data['XAUUSD']['open'], 10, 0.10)
    new_series, bins = piecewise_linear(reader.data['XAUUSD']['open'], 15)
